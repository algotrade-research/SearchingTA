...
class Backtesting(AbstractBacktesting):
    ...
    def close_order(self, curr_price, date) -> None:
        closed_positions = []
        pnl = 0.0
        ...
        for i, row in self._holdings.iterrows():
            should_close = False
            if row["signal"] == "buy":
                if curr_price >= row["TP"] or curr_price <= row["SL"]:
                    pnl += (curr_price - row["price"])
                    should_close = True
            else:  # sell
                if curr_price <= row["TP"] or curr_price >= row["SL"]:
                    pnl += (row["price"] - curr_price)
                    should_close = True
    ...
    def place_order(self, curr_price, signal, date) -> None:
        order = {
            "date": date,
            "price": curr_price,
            "signal": signal,
            "timeout": date + pd.Timedelta(minutes=30)
        }

        self.order_book = pd.concat([self.order_book, pd.DataFrame([order])], ignore_index=True)

    def check_order(self, curr_price, date) -> None:
        # check fillded orders
        filled = []
        self.order_book.sort_values("date", inplace=True, ascending=False)
        for i, row in self.order_book.iterrows():
            signal = row["signal"]
            if signal == 1 and curr_price >= row["price"]:
                self.open_order(curr_price, row["signal"], date)
                filled.append(i)
            elif signal == -1 and curr_price <= row["price"]:
                self.open_order(curr_price, row["signal"], date)
                filled.append(i)
    ...
    def _close_all(self, curr_price, date) -> None:
        pnl = 0
        for i, row in self._holdings.iterrows():
            row["close_price"] = curr_price
            row["pnl"] = (curr_price - row["price"]) if row["signal"] == "buy" else (row["price"] - curr_price)
            row["close_time"] = date
            ...
            for i in range(self.window, len(self.data) - 10):
                ...
                if time >= pd.to_datetime('09:15').time() and time <= pd.to_datetime('14:30').time():
                    ...
                    if len(self._holdings) > 0:
                        pnl = self.close_order(price, datetime)
                        if pnl != 0:
                    ...
                    if len(self.order_book) > 0:
                        self.check_order(price, datetime)

                if time >= pd.to_datetime('14:29').time():
                    pnl = self._close_all(self.data["price"].iloc[i], datetime)
                    self.data.loc[self.data.index[i], "Equity"] = pnl + self.data.loc[self.data.index[i], "Equity"] 


                # Update the progress bar
                pbar.set_postfix({
                    'Equity': f'{self.data.loc[self.data.index[i], "Equity"]:.2f}',
                ...
                if self.data.loc[self.data.index[i], "Equity"] < 8000:
                    break
    ...
    def evaluate(self) -> pd.DataFrame:
        ...
        vn30_result = pd.DataFrame({
            ...
            "annualized_return": (VN30F1[-1] / VN30F1[0]) ** (252 / len(VN30F1)) - 1
        }, index=["vn30"])
        ...
        result = pd.concat([vn30_result, strategy_result], axis=1)
