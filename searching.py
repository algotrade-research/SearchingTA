...
logging.basicConfig(filename='optimization_results.log', level=logging.INFO, format='%(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
strategy_options = [
    ...
    ('Volume_MA', Volume_MA), # 14
    ('MomentumBBL', MomentumBBL), # 15
    ("Keltner", Keltner), # 16
    ("SO", SO), # 17
    ('Williams R', W_R), # 18
    ('PSAR', PSAR), # 19
    ('OBV', OBV), # 20
    ('Donchian', Donchian), # 21
    ('UO', UO), # 22
    ('Force Index', FI), # 23
    ('Vortex', Vortex) # 24
]
...
def objective(trial):
    ...
    try:
        ...
        os.makedirs(os.path.join("optimize", subdir_name), exist_ok=True)
        backtester._history.to_csv(os.path.join("optimize", subdir_name, "history.csv"))
        
        file_path = os.path.join("optimize", subdir_name, "params.txt")
